...
def reduceTo20m(input_image):
    ...
    # defaultImage = ee.Image(ee.ImageCollection('COPERNICUS/S2_SR').first())
    # defaultBand = 'B5' # B5 is a 20m resolution band

    # set default projection to 20 m resolution
    defaultCrs = 'EPSG:32611' # defaultImage.select(defaultBand).projection().crs()
    defaultScale = 20 # defaultImage.select(defaultBand).projection().nominalScale()
